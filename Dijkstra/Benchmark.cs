...
namespace Dijkstra
{
    public partial class Benchmark : Form
    {
        ...
        private void CalculateExecTime(object args)
        {
            try
            {
                var data = (KeyValuePair<int, KeyValuePair<Node, Node>>)args;
                int rep = data.Key;
                ...
                for (int i = 0; i < rep; i++)
                {
                    this.CurrentGraph.DijkstraDistanceVector(data.Value.Key,data.Value.Value);
                }
                sw.Stop();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Você deve selecionar o nó de término.", "Erro ao calcular tempo de execução",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }            
            ...
            ExecTimeCalcThread.Start(new KeyValuePair<int, KeyValuePair<Node,Node>>(rep, new KeyValuePair<Node,Node>(start,end)));
        }
